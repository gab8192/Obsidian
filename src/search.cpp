...
namespace Search {
  ...
  DEFINE_PARAM_S(HistPrDepthMul, -4645, 400);
    ...
    while (move = movePicker.nextMove(skipQuiets)) {
      ...
      if (score > bestScore) {
        ...
        if (bestScore > alpha) {
          ...
          if (depth >= 2 && depth <= 10 && score > SCORE_TB_LOSS_IN_MAX_PLY)
            depth--;
        }
      }
