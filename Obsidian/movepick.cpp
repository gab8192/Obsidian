...
void MovePicker::scoreQuiets() {
  ...
  while (i < quiets.size()) {
    ...
    if (pt == QUEEN) {
      if (threats.byRook & from) threatScore += 16384;
      if (threats.byRook & to) threatScore -= 16384;
    } else if (pt == ROOK) {
      if (threats.byMinor & from) threatScore += 8192;
      if (threats.byMinor & to) threatScore -= 8192;
    } else if (pt == KNIGHT || pt == BISHOP) {
      if (threats.byPawn & from) threatScore += 8192;
      if (threats.byPawn & to) threatScore -= 8192;
    }
