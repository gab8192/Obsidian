...
namespace NNUE {
  ...
  struct {
    ...
    alignas(Alignment) weight_t OldOutputWeights[2 * HiddenWidth][OutputBuckets];
                       weight_t OutputBias[OutputBuckets];
  } Content;

  alignas(Alignment) weight_t NewOutputWeights[OutputBuckets][2 * HiddenWidth];
  ...
  Score evaluate(Position& pos, Accumulator& accumulator) {
    int numPc = BitCount(pos.pieces());
    int outputBucket = std::min((63 - numPc) * (32 - numPc) / 225, 7);
