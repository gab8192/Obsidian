...
namespace Search {
  ...
  Score SearchThread::qsearch(Position& pos, Score alpha, Score beta, SearchInfo* ss) {
    ...
    if (ply >= MAX_PLY-4)
      return pos.checkers ? DRAW : Eval::evaluate(pos, accumulator);
    ...
    if (pos.checkers) {
      ...
      if (ttHit)
        ...
        bestScore = ss->staticEval = Eval::evaluate(pos, accumulator);
    ...
    while (move = movePicker.nextMove(&moveStage)) {
      ...
      cancelMove();
      pos.undoMoveOnAcc(move, accumulator);
  ...
  Score SearchThread::negaMax(Position& pos, Score alpha, Score beta, int depth, bool cutNode, SearchInfo* ss) {
    ...
    if (pos.checkers) {
      ...
      if (ttHit)
        ...
        ss->staticEval = eval = Eval::evaluate(pos, accumulator);
  ...
  void SearchThread::startSearch() {
    ...
    rootPos.updateAccumulator(accumulator);
